...
contract ValeuPost is ERC721, ERC721URIStorage {
    ...
    constructor() ERC721("ValeuPost", "VALEU") {}
    ...
    function publish(string memory tokenURI_, bytes32 contentHash) external returns (uint256 tokenId) {
        require(bytes(tokenURI_).length != 0, "ValeuPost: tokenURI required");
        require(contentHash != bytes32(0), "ValeuPost: contentHash required");
